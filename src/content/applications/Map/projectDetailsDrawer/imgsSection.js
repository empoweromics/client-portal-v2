...
const ImgsSection = ({ projectDetails }) => {
    return (
        <div className={styles.header_imgs_wrapper}>
            <img onError={handleError} className={styles.cover_img} alt='' src={`https://empoweromics.com/app/pp/${projectDetails?.project?._id?.split('-')[1]}`} />
            <div className={` ${styles.upper_logo_wrapper}`}>
                <img alt='' src={`https://empoweromics.com/app/pl/${projectDetails?.project?._id?.split('-')[1]}`} className={styles.top_logo} />
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <div style={{ marginY: '0' }} className={styles.sub_title_text}>
                        By  {projectDetails?.project?.i18n?.en?.name}
                    </div>
                    
                    <div style={{ marginY: '0' }} className={styles.sub_title_text}> {projectDetails?.project?.state}</div>
                </div>
            </div>
            <div className={` ${styles.lower_logo_wrapper}`}>
                {/* <Rating
                    ...
                    readOnly
                /> */}
                <div className={styles.sub_title_text} style={{display:'flex',flexDirection:"column",alignItems:'flex-end'}}>
            ...
            <div className={styles.sub_title_text + ' ' + styles.units_div}> {projectDetails?.project?.units?.totla} Units</div>
